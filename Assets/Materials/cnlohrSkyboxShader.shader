...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        half calcSunAttenuation(half3 lightPos, half3 ray)
        {
			...
			delta = abs(delta);
            half dist = max( max( delta.x, delta.y ), max( delta.x, delta.z ) );//length(delta);
        ...
        // if y < 0 [eyeRay.y > 0] - sky
			...
			if(ray.y < 0.0)
            {
				...
				{
					float moonness = calcSunAttenuation(_WorldSpaceLightPos0.xyz, ray);
					col += saturate( 10 * moonness ) * .5;
					if( moonness <= 0 )
					{
						float nightness = saturate( .1-_WorldSpaceLightPos0.y ) * 3.;
						col += StarCalc( -ray, _WorldSpaceLightPos0.xyz )*nightness;
					}
				}
            }
        #endif
			...
			col.xyz += hash13( float3( _Time.y, Pos.xy ) )/512.-1./1024;
